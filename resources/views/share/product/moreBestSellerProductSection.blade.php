...
@if(isset($getHotProduct) && count($getHotProduct) > 0)
                        ...
                        <div class="tab-content">
                            <div class="ss-carousel ss-owl">
                                <div style="top: -15px;" class="owl-carousel"
                                    ...
                                    @foreach($getHotProduct as $productKey => $product)
                                                            ...
                                                            <div class="product-image-container product-image">
                                                                <a class="grid-view-item__link image-ajax" href="{{ (Route::has('productDetails') ? Route('productDetails', ['productID'=>str_replace(' ', '_+_', $product->product_name)]) : 'javascript:;') }}">
                                                                    ...
                                                                    <img class="img-responsive s-img lazyload" data-sizes="auto" style="max-height: 210px;"
                                                                        ...
                                                                        data-src="@if(isset($getHotPath[$productKey]) && isset($getHotPath300x300) && isset($getHotPath500x500))
                                                                                @if(@getimagesize($getHotPath[$productKey] . $getHotPath300x300 . $getHotCoverImage[$productKey]))
                                                                                    {{$getHotPath[$productKey] . $getHotPath300x300 . $getHotCoverImage[$productKey]}}
                                                                                @elseif(@getimagesize($getHotPath[$productKey] . $getHotPath500x500 . $getHotCoverImage[$productKey]))
                                                                                    {{$getHotPath[$productKey] . $getHotPath500x500 . $getHotCoverImage[$productKey]}}
                                                                                @else
                                                                                    {{$getHotPath[$productKey] . $getHotCoverImage[$productKey]}}
                                                                                @endif
                                                                            @endif"
                                                                        alt="{{ $product->product_name }}" />
                                                                </a>
                                                                <ul class="product-card__bottom product-card__gallery">
                                                                    @if(isset($getHotImages) && $getHotImages)
                                                                        @foreach($getHotImages[$productKey] as $imageKey => $productImage)
                                                                            <li class="item-img {{ (($imageKey == 0) ? 'thumb-active' : '') }}" data-src="{{ (isset($getHotPath) && isset($getHotPath300x300)) ? $getHotPath[$productKey] . $getHotPath300x300 . $productImage->file_name : null }}">
                                                                                <img class="lazyload" data-sizes="auto" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="{{ (isset($getHotPath) && isset($getHotPath300x300)) ? $getHotPath[$productKey] . $getHotPath300x300 .  $productImage->file_name : null}}" alt="{{ $productImage->file_description }}">
                                                                            </li>
                                                                        @endforeach
                                                                    @endif
                                                                </ul>
